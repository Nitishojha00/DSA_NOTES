class Solution {
public:
// i will do it later exam date colliding
    string shortestCommonSupersequence(string str1, string str2) {
        ...
        while (i > 0 && j > 0) {
            if (str1[i - 1] == str2[j - 1]) {
                ...
                // If coming from top has higher value, take character from str1 why??
                // beacuse we coming from already maximum possible subsequence if anyother coming and
                //  its greater in path means we didnt consider in real lcs thats why we added 
                result.push_back(str1[i - 1]);
                i--;
            } else {
